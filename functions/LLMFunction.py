...
class LLMFunction(AbstractFunction):
    ...
    @forward(
        input_signatures=[
            PandasDataframe(
                columns=["query", "requirement", "points"],
                column_types=[
                    NdArrayType.STR,
                    NdArrayType.STR,
                    NdArrayType.STR,
    ...
    def forward(self, text_df):
        ...
        for i in range(len(llm_system)):
            for index, row in text_df.iterrows():
                ...
                full_prompt = f"[no prose] \n Here is the criteria for grading: {requirement},\n Total score out of {points},\n {query}"
                system = "You are Teaching Assistant and {} grader.".format(llm_system[i][0])
                response = try_llm_prompt(full_prompt, system=system, temperature=float(llm_system[i][1]))
                
                if (response.isdigit()):
                    print("llm response for rubric no." + str(index+1) +" : by ", llm_system[i][0], " grader, ", response, "/", points)
                    # print("========================================")

                    # values_list.append(
                    #     {"ids": str(i), "score": response}
                    # )
                    ...
                    try:
                        ...
                        trial = 3
                        while trial != 0:
                            trial_response = try_llm_prompt(full_prompt, system, float(llm_system[i][1]))
                            if trial_response.isdigit():
